...
$hID = $_SESSION["hID"];
$numItem = $_SESSION["reqItem"];
echo "session hID: ".$_SESSION["hID"];
echo "<br>";
echo "hID: ".$hID;
echo "<br>";
...
for($i=0;$i<$numItem;$i++){
	$j = $i+1;
  if(isset($_SESSION["detailID"][$j])){
    echo "Detail ID ".$_SESSION["detailID"][$j];
    echo "<br>";
    $dID = $_SESSION["detailID"][$j];
  }
  if(isset($_SESSION["newQty"][$j])){
    echo "Qty ".$_SESSION["newQty"][$j];
    echo "<br>";
    $Qty = $_SESSION["newQty"][$j];

    if($objCon->connect_errno)
    {
      echo $objCon->connect_errno.": ".$objCon->connect_error;
    }
    $q="UPDATE detail SET Quantity = '$Qty' where Detail_ID = '$dID'";
    if(!$objCon->query($q))
    {
      echo "UPDATE failed. Error: ".$objCon->error ;
    }
  }
  if(isset($_SESSION["stat"][$j])){
    echo "Stat ".$_SESSION["stat"][$j];
    echo "<br>";
    $stat = $_SESSION["stat"][$j];

    if($objCon->connect_errno)
    {
      echo $objCon->connect_errno.": ".$objCon->connect_error;
    }
    $q="UPDATE detail SET status = '$stat' where Detail_ID = '$dID'";
    if(!$objCon->query($q))
    {
      echo "UPDATE failed. Error: ".$objCon->error ;
    }
...
$q="UPDATE history SET status = 'Approved' where ID = '$hID'";
if(!$objCon->query($q))
{
  echo "UPDATE failed. Error: ".$objCon->error ;
}
...
$fromstock = $_SESSION["fromstock"];
	...
	$strSQL = "SELECT Item_ID, Quantity FROM detail WHERE Order_ID = $hID";
				...
				if($value >= $result){ // item of row x more than wanted number
					...
					if($result == 0){ // item in row x = wanted number
						...
						if($cond == 1 && $firsttime != 0){	//รอบสุดท้ายลบกันได้0
							...
							$q="INSERT INTO detail (Type, Order_ID, Item_ID, Quantity, CostPerUnit, status)
								...
								FROM detail
								WHERE Order_ID = $hID AND Item_ID = $getID LIMIT 1;";
							if(!$objCon->query($q))
							{
						...
						elseif($cond == 0 || $firsttime == 0){	//รอบแรก
							...
							$q = "UPDATE detail SET Quantity = $value, CostPerUnit = $getEachPrice[$key] where Order_ID = $hID AND Item_ID = $getID";
							if(!$objCon->query($q))
							{
								echo "UPDATE failed. Error: ".$objCon->error ;
							}
						...
						elseif($cond == 0 || $firsttime == 0){	//รอบแรก
							...
							$q = "UPDATE detail SET Quantity = $total, CostPerUnit = $getEachPrice[$key] where Order_ID = $hID AND Item_ID = $getID";
							if(!$objCon->query($q))
							{
								echo "UPDATE failed. Error: ".$objCon->error ;
							}
					...
					if($checkzero < 0){
						...
						if($firsttime == 0 && $value != 0){	//เข้าลูปนี้รอบแรก
							...
							$q = "UPDATE detail SET Quantity = $value, CostPerUnit = $getEachPrice[$key] where Order_ID = $hID AND Item_ID = $getID ORDER BY Detail_ID DESC LIMIT 1";
							if(!$objCon->query($q))
							{
								echo "UPDATE failed. Error: ".$objCon->error ;
							}
